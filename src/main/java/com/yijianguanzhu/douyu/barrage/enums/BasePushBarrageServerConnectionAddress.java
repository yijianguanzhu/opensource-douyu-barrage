 ...
 */
public interface BasePushBarrageServerConnectionAddress extends BaseBarrageServerConnectionAddress {

	BasePushBarrageServerConnectionAddress PUSH_ADDRESS = () -> null;

	// 地址读指针
	AtomicInteger READERINDEX = new AtomicInteger( 0 );

	// 所有地址
	List<BasePushBarrageServerConnectionAddress> ADDRESSS = new CopyOnWriteArrayList<>();
	...
	default BaseBarrageServerConnectionAddress next() {
		if ( ADDRESSS.isEmpty() ) {
			Collections.addAll( ADDRESSS, DefaultPushConnectionAddress.values() );
		}
